...
import { Sparkles, Menu, X, Home, MessageSquare, Image as ImageIcon, Layers } from "lucide-react";
      ...
      >
        <div className={`flex items-center gap-1 rounded-full border border-white/10 bg-black/50 p-1.5 shadow-2xl backdrop-blur-xl transition-all duration-300 ${scrolled ? "bg-black/80 border-white/5" : ""}`}>
          ...
          <div className="ml-2 pl-2 border-l border-white/10">
             <button className="rounded-full bg-white px-4 py-2 text-xs font-bold text-black transition-transform hover:scale-105 active:scale-95">
               Pro
             </button>
          </div>
        </div>
